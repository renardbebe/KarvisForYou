# -*- coding: utf-8 -*-
"""
KarvisForYou V13 统一存储接口
支持 Local / OneDrive 两种后端，通过工厂方法按用户配置创建。

Duck Typing 协议 — LocalFileIO 和 OneDriveIO 都实现以下方法（实例方法）：
  get_token()
  read_text(path) -> str
...
def create_storage(storage_mode: str = "local", onedrive_config: dict = None):
    """工厂方法：根据模式创建存储实例。
    ...
    Returns:
        LocalFileIO 实例 或 OneDriveIO 实例（统一返回实例）
    """
